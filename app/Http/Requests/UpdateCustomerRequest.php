...
class UpdateCustomerRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // Request fields - reference is required to identify customer
            'request.reference' => ['required', 'string', 'max:45'],
            
            // Fields that cannot be updated (required but values must match existing)
    ...
    public function messages(): array
    {
        return [
            ...
            'request.reference.required' => 'Reference is required to identify the customer.',
            'request.reference.max' => 'Reference must not exceed 45 characters.',
            
            'request.customerAccount.required' => 'Customer account is required (must match existing value).',
            'request.customerAccount.max' => 'Customer account must not exceed 45 characters.',
            
            'request.accountType.required' => 'Account type is required (must match existing value).',
            'request.accountType.in' => 'Account type must be BA (Bank Account) or DW (Digital Wallet).',
            
            'request.destinationBank.required' => 'Destination bank is required (must match existing value).',
